...
export function CatalogueTile({ flavor, isTasted, onToggle }: CatalogueTileProps) {
  ...
  return (
    <li className={`${isArchived ? "bg-archived" : "bg-background"} rounded-3xl border-none`}>
      <button
        type="button"
      ...
      >
        ...
        <div className="flex flex-wrap justify-center gap-1">
          {isTasted ? (
            // Badge goûtée (`badge-tasted`, DESIGN.md) — porté par du texte, pas
            // seulement par la couleur. Le fond de la tuile reste neutre
            // (`bg-background`/`bg-archived`), jamais de remplissage plein
            // vert : anti-pattern "feu tricolore" explicitement rejeté.
            <span className="bg-success text-success-foreground rounded-full px-2 py-0.5 text-xs">
              Goûtée
            </span>
          ) : null}
          {isArchived ? (
